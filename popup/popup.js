...
async function changeN(delta) {
  ...
  await browser.storage.local.set({ lokN: state.n });
  renderN();
}
...
async function init() {
  ...
  const s = await browser.storage.session.get(["lokClipStart", "lokClipEnd", "lokProgress"]);
  ...
  // N parallèle
  const ls = await browser.storage.local.get("lokN");
...
document.getElementById("btn-set-start").addEventListener("click", async () => {
  ...
  if (!tab) return;
  const resp = await browser.tabs.sendMessage(tab.id, { action: "get_time" }).catch(() => null);
  if (resp?.time !== undefined) {
    state.clipStart = resp.time;
    await browser.storage.session.set({ lokClipStart: state.clipStart });
    updateUI();
...
document.getElementById("btn-set-end").addEventListener("click", async () => {
  ...
  if (resp?.time !== undefined) {
    ...
    await browser.storage.session.set({ lokClipEnd: state.clipEnd });
    updateUI();
...
document.getElementById("btn-dl-full").addEventListener("click", async () => {
  ...
  const resp = await browser.runtime.sendMessage({ action: "download_full", url: state.videoUrl, n: state.n });
  if (resp.status === "ok") {
